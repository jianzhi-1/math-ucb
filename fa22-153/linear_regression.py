def linear_regression(X, y):
    ...
    sigmahat = np.sqrt((1./(n - p))*np.linalg.norm(y - X@betahat)**2)
    beta_confidence_interval = []
    sigma_confidence_interval = None
    
    for i in range(len(betahat)):
        beta_confidence_interval.append(stats.t.interval(0.95, n - p, loc=betahat[i], scale=sigmahat*np.sqrt(np.linalg.inv(X.T@X)[i][i])))
